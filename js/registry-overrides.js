...
class RegistryOverrides {
  static SCRIPT = {
    ...
    author: "Bluscream",
    version: "1.0.0",
    build: "{build:registry-overrides.js}",
    dependencies: [
    ...
    try {
      ...
      for (const [key, configValue] of Object.entries(config)) {
        try {
          ...
          window.Logger?.log(
            `[${triggerEvent}] ${key} was ${oldVal}, setting to ${value}`,
            { console: true },
            "info"
          );
          ...
          window.Logger?.log(
            `Error setting registry key ${key}: ${error.message}`,
            { console: true },
            "error"
          );
        }
      ...
      window.Logger?.log(
        `Error applying registry settings: ${error.message}`,
        { console: true },
        "error"
      );
    }
...
(function () {
  ...
  console.log(
    `✓ Loaded ${RegistryOverrides.SCRIPT.name} v${RegistryOverrides.SCRIPT.version} by ${RegistryOverrides.SCRIPT.author}`
  );
})();
